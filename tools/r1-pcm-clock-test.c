...
/* Minimal freestanding ALSA PCM zero-stream test for the Phicomm R1. */
...
#define DEFAULT_SECONDS 20U
#define MAX_SECONDS 120U
...
static void set_mask(struct snd_pcm_hw_params *params, u32 parameter,
		     u32 value)
{
...
static int configure_pcm(long fd)
{
	...
	set_mask(&params, SNDRV_PCM_HW_PARAM_SUBFORMAT,
...
static __used __noinline int main(int argc, char **argv)
{
	...
	if (argc > 2) {
		putstr("usage: r1-pcm-clock-test [seconds: 1..120]\n");
		return 2;
	}
	if (argc == 2) {
		seconds = parse_seconds(argv[1]);
		if (!seconds) {
			putstr("seconds must be in range 1..120\n");
			return 2;
		}
	}
	...
	while (bytes_left) {
		...
		long ret = syscall3(4, fd, (long)zero_period, chunk);

		if (ret == -EPIPE) {
			xruns++;
			ret = syscall3(54, fd, SNDRV_PCM_IOCTL_PREPARE, 0);
			if (ret < 0) {
				put_error("xrun recovery", ret);
				(void)syscall1(6, fd);
				return 1;
			}
			continue;
		...
		if (ret < 0) {
			put_error("PCM zero write", ret);
			(void)syscall3(54, fd, SNDRV_PCM_IOCTL_DROP, 0);
			(void)syscall1(6, fd);
			return 1;
		}
		if (!ret) {
			putstr("PCM zero write returned zero\n");
			(void)syscall3(54, fd, SNDRV_PCM_IOCTL_DROP, 0);
			(void)syscall1(6, fd);
			return 1;
		}
