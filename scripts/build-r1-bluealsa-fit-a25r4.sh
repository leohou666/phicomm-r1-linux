...
# Keep FIT node timestamps deterministic across repeated host builds.
...
its="$project_root/scripts/r1-linux-mainline-6.18-ak7755-bluealsa-a25r4.its"
fit="$artifacts/r1-linux-mainline-6.18-ak7755-bluealsa-a25r4.itb"
...
[ -f "$kernel" ] && [ -f "$dtb" ] && [ -f "$rootfs" ] || {
	echo "Missing A24r2 kernel/DTB or Buildroot rootfs." >&2
	echo "Run scripts/build-r1-ak7755-auto-amp-a24r2.sh and JOBS=16 scripts/build-r1-bluealsa-rootfs.sh first." >&2
	exit 1
}
...
etc/init.d/S45r1-bluealsa-stack
usr/sbin/r1-bluealsa-supervisor
...
gzip -dc "$rootfs" | (
	cd "$rootfs_dir"
	# Extract only audited payloads; do not recreate initramfs device nodes.
...
for script in etc/init.d/S45r1-bluealsa-stack usr/sbin/r1-bluealsa-supervisor; do
	[ -x "$rootfs_dir/$script" ] || { echo "rootfs script is not executable: $script" >&2; exit 1; }
	sh -n "$rootfs_dir/$script"
done

for stale in etc/init.d/S45bluealsa etc/init.d/S50bluealsa-aplay; do
	if gzip -dc "$rootfs" | cpio -t --quiet 2>/dev/null | grep -qx "$stale"; then
		echo "rootfs retains stale service: $stale" >&2
		exit 1
	fi
done
...
echo "A25r4 supervised BlueALSA FIT verified; eMMC was not modified."
